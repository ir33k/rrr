...
{
	OK(0 == uri_protocol(""));
	OK(0 == uri_protocol("test:70/1/path"));
	OK(0 == uri_protocol("://test:70/1/path"));
	OK(0 == uri_protocol("://"));
	OK(0 == uri_protocol("aaa://test:70/1/path"));
	OK(0 == uri_protocol("gopherR://test:70/1/path"));
	OK(GOPHER == uri_protocol("gopher://test"));
	OK(GOPHER == uri_protocol("Gopher://test"));
	OK(GOPHER == uri_protocol("GOPHER://test"));
	OK(GEMINI == uri_protocol("gemini://test"));
	OK(HTTPS  == uri_protocol("https://test"));
	OK(HTTP   == uri_protocol("http://test"));
	OK(HTTP   == uri_protocol("    http://test"));
	OK(HTTP   == uri_protocol("GET http://test"));
	OK(HTTP   == uri_protocol("SOMETHING http://test"));
	OK(GEMINI == uri_protocol("LOREM gemini://test"));
}
